...
async function loadCabinetProfile() {
    try {
        ...
        document.getElementById('contact-phone').textContent = d.cabinet.phone || '—';
        document.getElementById('contact-email').textContent = d.cabinet.email || '—';
...
function renderRatingBreakdown(breakdown, total) {
    ...
    if (bigRating && total > 0) {
        const sum = Object.keys(breakdown).reduce((acc, key) => acc + (parseInt(key) * breakdown[key]), 0);
...
function renderDoctors(doctors) {
    ...
    let html = '';
    doctors.forEach(doc => {
        const initials = doc.name.split(' ').slice(1).map(n => n[0]).join('').substring(0, 2).toUpperCase();
        html += `
            ...
            <div class="review-card ${hideClass}">
                <div class="review-header">
                    <div class="reviewer-info">
                        <div class="reviewer-avatar" style="background: #0891b2; color: white; width: 50px; height: 50px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold;">${escapeHtml(rev.username.substring(0, 2).toUpperCase())}</div>
                        <div>
...
function renderSpecializations(cabinetSpecialty, doctors) {
    ...
    list.innerHTML = Array.from(specialties).map(s => `<li>${escapeHtml(s)}</li>`).join('');
}

function renderPricing(pricing) {
    const pricingBox = document.querySelector('.cabinet-detail-box:nth-child(1) ul');
    ...
    pricingBox.innerHTML = pricing.map(p => 
        `<li>${escapeHtml(p.service)}: <strong>${p.price.toFixed(2)} DZD</strong></li>`
    ).join('');
}
...
function showDoctorProfile(doctorId) {
    ...
    const initials = doctor.name.split(' ').slice(1).map(n => n[0]).join('').substring(0, 2).toUpperCase();
    ...
    modalImage.insertAdjacentHTML('afterend', `<div class="modal-doctor-avatar" style="width: 150px; height: 150px; background: linear-gradient(135deg, #0891b2, #06b6d4); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 3rem; font-weight: bold; margin-bottom: 1rem;">${initials}</div>`);
