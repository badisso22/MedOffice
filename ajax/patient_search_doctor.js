...
async function onCriteriaChange() {
  ...
  document.getElementById("facilitiesSelector").style.display = wizardState.selectedCriteria.includes("facilities")
    ? "block"
    : "none"

  document.getElementById("locationSelector").style.display = wizardState.selectedCriteria.includes("location")
    ? "block"
    : "none"
}
...
function updatePreferences(type) {
  if (type === "facilities") {
    const checked = Array.from(document.querySelectorAll('input[name="facility"]:checked')).map((cb) => cb.value)
...
function selectMethod(method) {
  ...
  if (method === "wsm") {
    ...
    methodInfo.innerHTML =
      "<p><strong>TOPSIS</strong> (Technique for Order of Preference by Similarity to Ideal Solution) compares each doctor to ideal and anti-ideal solutions. It's more sophisticated and handles complex multi-criteria decisions better.</p>"
  }
}
