...
const icons = {
  ...
  cross: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>`,
  doctor: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 14.66v-5.33A6 6 0 0 0 9.34 6m0 0H6a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2h-3.34M9 9h0M12 12h0"></path></svg>`
};
...
async function loadDoctorsList() {
  try {
    const res = await fetch("../api/get-doctors.php");
    const json = await res.json();
...
function renderCalendar() {
  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    html += `
      <div class="cal-day-cell ${isToday ? "today" : ""} ${dayApts.length > 0 ? "has-appointments" : ""} ${
      isPast ? "past-date" : ""
    }" 
           data-date="${dateKey}"
            ...
            ${dayApts
              ...
              <div class="day-apt-card" style="border-left: 4px solid ${getColor(apt.type)};">
                <div class="day-apt-header">
                  ...
                  apt.id
                }); closeDayApptsModal();">Edit</button>
                    <button class="day-apt-btn-delete" onclick="confirmDelete('${date}', ${
                  apt.id
                })">${icons.trash}</button>
                  </div>
                </div>
...
function goToStep(step) {
  ...
  if (step === 1) {
    ...
    nextBtn.style.display = "inline-block";
    nextBtn.textContent = step === 4 ? "Book Appointment" : "Next →";
  } else if (step === 5) {
...
async function populateDoctorsGrid() {
  ...
  try {
    ...
    if (selectedDate) {
      ...
      doctors = allDoctors.filter((d) => d.dayOfWeek === weekday);
      if (doctors.length === 0) {
        container.innerHTML = "<p>No doctors available for this date.</p>";
        return;
      }
    }
      ...
      card.innerHTML = `
        <div class="doctor-header">
          <div class="doctor-avatar">
            ${escapeHtml(doctor.firstName.charAt(0) + doctor.lastName.charAt(0))}
          </div>
          <div class="doctor-info">
            <div class="doctor-name">${escapeHtml(doctor.firstName + " " + doctor.lastName)}</div>
            <div class="doctor-specialty">${escapeHtml(doctor.specialty)}</div>
          </div>
        </div>
        <div class="doctor-hours">
          ${doctor.startTime} - ${doctor.endTime} (${doctor.dayOfWeek})
        </div>
      `;
      card.addEventListener("click", () => selectDoctor(card, doctor));
      container.appendChild(card);
    });
  } catch (err) {
    console.error("Error loading doctors:", err);
    container.innerHTML = "<p>Error loading doctors. Please try again.</p>";
  }
}

function selectDoctor(cardElement, doctor) {
  document.querySelectorAll(".doctor-card").forEach((card) => card.classList.remove("selected"));
  cardElement.classList.add("selected");
  document.getElementById("appointmentDoctor").value = doctor.doctorID;
  ...
  const infoCard = document.getElementById("doctorSelectedInfo");
  if (infoCard) {
    document.getElementById("selectedDoctorName").textContent = escapeHtml(
      doctor.firstName + " " + doctor.lastName
    );
    const years = doctor.yearsOfExperience ?? doctor.experience ?? "";
    document.getElementById("selectedDoctorSpecialty").textContent = `${escapeHtml(
      doctor.specialty
    )}${years ? " • " + years + " years" : ""}`;
    document.getElementById("selectedDoctorHours").textContent = `${doctor.startTime} - ${doctor.endTime}`;
...
async function generateTimeSlots() {
  ...
  if (selectedDate && appointments[selectedDate]) {
    bookedTimes = appointments[selectedDate]
  ...
  if (selectedDoctorInfo) {
    startHour = parseInt(selectedDoctorInfo.startTime.split(":")[0], 10);
    endHour = parseInt(selectedDoctorInfo.endTime.split(":")[0], 10);
  }
...
async function saveAppointment() {
  ...
  if (!patientId || !date || !time || !type || !doctorId) {
    ...
    <div class="modal-overlay active" id="deleteConfirmOverlay" style="z-index: 3000;">
      <div class="modal" style="max-width: 400px;">
        ...
        <div class="modal-footer">
          ...
          <button class="btn-modal" style="background: #ef4444; color: white;" onclick="deleteAppointment(${id})">
            Delete Appointment
          </button>
        </div>
      </div>
